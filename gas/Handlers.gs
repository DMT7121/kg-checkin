...
function buildEmailHtml(payload, formattedTimeUI, loc, distMeters, isValid, isAdmin) {
  ...
  var badgeText = isAdmin ? 'PHÁT SINH LƯỢT CHẤM CÔNG' : typeStr.toUpperCase() + ' THÀNH CÔNG';
  
  var bannerBg = isValid
    ? 'linear-gradient(135deg, #38e98d 0%, #0abc56 54%, #16c971 100%)'
    : 'linear-gradient(135deg, #ff6b6b 0%, #ee2a2a 54%, #d61818 100%)';
  var bannerShadow = isValid
    ? '0 8px 20px rgba(22,195,103,.30)'
    : '0 8px 20px rgba(195,22,22,.30)';
  ...
  var subtitleHTML = isAdmin 
    ...
    : 'Xin chào <b>' + fullnameStr + '</b>,<br/><span style="font-size:13px;color:#667894;font-weight:normal;display:block;margin-top:4px;">' + greeting + '</span>';
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    body {
      ...
      background: linear-gradient(180deg, #eef4ff 0%, #f3f4f6 100%);
      color: #06285d;
    }
    .stage {
      max-width: 480px;
    ...
    .poster {
      ...
      border-radius: 20px;
      background: linear-gradient(145deg, rgba(255,255,255,.97) 0%, rgba(241,248,255,.96) 50%, rgba(233,243,255,.98) 100%);
      border: 1px solid rgba(255,255,255,.9);
      box-shadow: 0 12px 40px rgba(9,54,126,.15);
    }
    ...
    .logo {
      ...
      border-radius: 14px;
      color: #ffd65a; font-size: 22px; font-weight: 900; letter-spacing: -1px;
      background: linear-gradient(145deg, #1664ff 0%, #003cbd 56%, #00217e 100%);
      box-shadow: 0 8px 20px rgba(0,58,169,.30);
    }
    h1 {
    ...
    .subtitle {
      ...
      margin-top: 8px; color: #71829f; font-size: 12px; font-weight: 500;
    }
    .subtitle::before, .subtitle::after {
      content: ""; width: 28px; height: 1px;
      background: linear-gradient(90deg, transparent, rgba(73,113,180,.3));
    }
    .subtitle::after { background: linear-gradient(90deg, rgba(73,113,180,.3), transparent); }
    .banner {
    ...
    .check-circle svg { width: 20px; height: 20px; stroke-width: 3; }
    .banner strong {
      font-size: 14px; font-weight: 800; letter-spacing: .2px;
      line-height: 1.3;
    }
    .note {
      margin: 0 0 16px; color: #2b4265; font-size: 14px;
      font-weight: 500; text-align: center; line-height: 1.5;
    }
    .note b { color: #0d55ff; font-weight: 700; }
    .info-card {
      border-radius: 16px; padding: 4px 0; overflow: hidden;
      background: rgba(255,255,255,.75);
      border: 1px solid rgba(200,220,255,.4);
      box-shadow: 0 8px 24px rgba(35,81,143,.08);
    }
    .row {
      display: flex; align-items: center; gap: 10px;
      padding: 10px 14px;
      border-bottom: 1px solid rgba(18,67,133,.08);
    }
    ...
    .icon-box {
      ...
      border: 1px solid rgba(255,255,255,.9);
      box-shadow: 0 4px 10px rgba(28,95,195,.08);
      color: #2365f4;
    }
    .icon-box svg { width: 18px; height: 18px; stroke-width: 2.2; }
    .row:last-child .icon-box { color: ${statusColor}; }
    .label {
      flex: 1; color: #40536f; font-size: 13px; font-weight: 600;
    }
    .value {
      text-align: right; color: #071f4b; font-size: 13px;
      font-weight: 700; line-height: 1.3; max-width: 180px;
      word-break: break-word;
    }
    .value.action { color: #0c55f4; font-size: 14px; font-weight: 800; }
    .value.address { font-weight: 500; font-size: 12px; line-height: 1.4; }
    .value.ok { color: ${statusColor}; font-size: 14px; font-weight: 800; }
    .dashboard {
      display: block; width: 220px; max-width: 80%; margin: 20px auto 16px;
      padding: 12px 0; border-radius: 12px;
      text-align: center; text-decoration: none;
      color: white; font-size: 14px; font-weight: 800;
      background: linear-gradient(180deg, #338dff 0%, #0055f4 51%, #0039b4 100%);
      box-shadow: 0 8px 20px rgba(0,64,180,.28);
    }
    .footer {
      text-align: center; margin-top: 12px; padding-top: 12px;
      border-top: 1px solid rgba(18,67,133,.08);
    }
    .footer p { margin: 0 0 4px; color: #667898; font-size: 11px; font-weight: 500; }
    .footer strong { color: #08275c; font-size: 12px; font-weight: 800; letter-spacing: 2px; }
  </style>
</head>
<body>
  <div class="stage">
    <section class="poster">
      ...
      <a class="dashboard" href="${CONFIG.WEB_APP_URL || '#'}" target="_blank">Truy cập Dashboard</a>
