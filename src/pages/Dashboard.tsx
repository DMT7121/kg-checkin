...
import { Sun, Moon, Power, Camera, Calendar, Clock3, ShieldAlert, ArrowLeftRight, Newspaper, GraduationCap } from 'lucide-react';
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-white dark:bg-gray-900 shadow-2xl relative overflow-hidden flex flex-col">
      ...
      <div className="px-3 pt-2 bg-white dark:bg-gray-900 sticky top-[76px] z-30">
        <div className="flex overflow-x-auto space-x-1 hide-scrollbar p-1 bg-gray-100 dark:bg-gray-800 rounded-xl snap-x">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            const isActive = currentTab === tab.id;
            return (
              <button key={tab.id} onClick={() => handleTabChange(tab.id)}
                className={`flex-shrink-0 w-20 py-2.5 rounded-lg text-[10px] sm:text-xs font-bold transition-all text-center touch-manipulation flex flex-col items-center justify-center min-h-[50px] relative snap-center ${isActive ? 'bg-white dark:bg-gray-700 text-ocean-600 dark:text-white shadow-sm ring-1 ring-black/5' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}>
